...
void nurse_iteration()
{
   ...
   if(patient == MAX_ID){	//independentemente do numero de pacientes o ultimo a ser observado pela nurse é sempre o paciente dummy, 
   	break;			//porque eu lanço a thread do dummy antes da dos pacientes, logo o dummy vai ser o ultimo paciente atendido pela nurse.
   }
...
void *nurse(void *argn){
	...
	nurse_iteration();
	printf("\e[35;01mNurse finish\e[0m\n");
...
int main(int argc, char *argv[])
{
   ...
   /* init simulation */
   init_simulation(npatients + 1); //npatients + dummy
	...
	pthread_t pthr[npatients+1]; 	// dummy and patients' ids
	ARGV p_ARGV[npatients+1];
	
 	p_ARGV[npatients].id = MAX_ID;
 	
	thread_create(&pthr[npatients],NULL,patient,&p_ARGV[npatients]);
	...
	for (unsigned int i = 0; i < ndoctors; i++){
   	...
   	/* wait for threads to conclude */
		...
		printf(">> Dummy thread has terminated <<\n");
